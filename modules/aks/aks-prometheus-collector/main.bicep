...
param enableWindowsRecordingRules bool = true
param grafanaName string
param grafanaResourceGroupName string 
param amwName string
param amwResourceGroupName string
...
resource azureMonitorWorkspace 'microsoft.monitor/accounts@2023-04-03' existing = {
  name: amwName
  scope: resourceGroup(amwResourceGroupName)
}
...
module azuremonitormetrics_dcra_clusterResourceId '../../nested_azuremonitormetrics_dcra_clusterResourceId.bicep' = {
  name: 'azuremonitormetrics-dcra-${uniqueString(aksCluster.id)}'
  scope: resourceGroup()
  params: {
...
module azuremonitormetrics_profile_clusterResourceId '../../nested_azuremonitormetrics_profile_clusterResourceId.bicep' = {
  name: 'azuremonitormetrics-profile--${uniqueString(aksCluster.id)}'
  scope: resourceGroup()
  params: {
    variables_clusterName: clusterName
    clusterLocation: location
...
//   name: guid(subscription().id, grafanaResourceName, '22926164-76b3-42b3-bc55-97df8dab3e41')
...
// Provide Grafana access to the AMW instance using Grafana Role assignment module
module grafana_role_assignment '../../grafanaRoleAssignment.bicep' = {
  name: 'grafana-role-assignment-${uniqueString(grafanaResource.id)}'
  scope: resourceGroup(grafanaResourceGroupName)
  params: {
    grafanaName: grafanaName
    amwName: amwName
    principalId: grafanaResource.identity.principalId
    roleGuid: 'b0d8363b-8ddd-447d-831f-62ca05bff136'
  }
  dependsOn: [
    grafanaResource
  ]
...
module admin_grafana_role_assignment '../../adminGrafanaRoleAssignment.bicep' = {
  name: 'admin_grafana-role-assignment-${uniqueString(grafanaResource.id)}'
  scope: resourceGroup(grafanaResourceGroupName)
  params: {
    grafanaName: grafanaName
    principalId: adminGroupObjectId
    roleGuid: '22926164-76b3-42b3-bc55-97df8dab3e41'
  }
  dependsOn: [
    grafanaResource
  ]
...
//Enable all Recommended Alerts
module recommendedMetricAlerts '../../recommendedMetricAlerts.bicep' = if(enablePrometheusRecommendedAlerts) {
  name: 'recommended_metric_alerts--${uniqueString(grafanaResource.id)}'
  scope: resourceGroup()
  params: {
    ...
    amwName: amwName
    clusterName: clusterName
    amwResourceGroupName: amwResourceGroupName
    location: location
  }
}
