...
@description('Flag to indicate if the subnet is existing.')
param isExistingSubnet bool = false
...
resource subnet 'Microsoft.Network/virtualNetworks/subnets@2022-07-01' = if (!isExistingSubnet) {
  name: subnetName
...
resource existingSubnet 'Microsoft.Network/virtualNetworks/subnets@2022-07-01' existing = if (isExistingSubnet) {
  name: subnetName
  parent: virtualNetwork
}

resource updateSubnet 'Microsoft.Network/virtualNetworks/subnets@2022-07-01' = if (isExistingSubnet) {
  name: subnetName
  parent: virtualNetwork
  properties: {
    routeTable: {
...
output subnetId string = isExistingSubnet ? existingSubnet.id : subnet.id
